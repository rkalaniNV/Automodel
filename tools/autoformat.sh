#!/bin/bash
# copied from https://github.com/NVIDIA/Megatron-LM/blob/main/tools/autoformat.sh
set -euox pipefail
...
CHANGED_FILES=$(git diff --name-only --diff-filter=d --merge-base origin/${BASE_REF} nemo_automodel/ recipes/ | grep '\.py$' || true)
ADDITIONAL_ARGS=""
