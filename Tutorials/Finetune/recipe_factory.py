...
from data_modules import LmsysHFDataModule
...
def create_lora_recipe(config: TrainingConfig) -> run.Config:
    ...
    recipe.data = run.Config(
        ...
        global_batch_size=config.compute.gpus_per_node*config.compute.nodes*config.data.micro_batch_size,
    ...
    recipe.trainer.strategy = run.Config(nl.FSDP2Strategy, data_parallel_size=config.compute.gpus_per_node*config.compute.nodes, tensor_parallel_size=1)
