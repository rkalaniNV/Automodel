...
def validate_config(config: TrainingConfig) -> None:
    ...
    if config.compute.use_slurm:
        required_slurm_fields = ["account", "partition", "remote_job_dir", "nodes", "gpus_per_node"]
...
def main():
    ...
    parser.add_argument(
        "--recipe-type", choices=["lora", "full"], default="lora", help="Type of training recipe to use"
    )
    parser.add_argument("--dry-run", action="store_true", help="Validate configuration without running training")
    parser.add_argument(
