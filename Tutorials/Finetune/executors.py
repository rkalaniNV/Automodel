...
def create_local_executor(config: TrainingConfig) -> run.LocalExecutor:
    ...
    env_vars.update(
        {"HF_HOME": config.model.cache_dir, "HF_TOKEN": config.model.token, "TRITON_CACHE_DIR": config.model.cache_dir}
    )

    executor = run.LocalExecutor(ntasks_per_node=config.compute.gpus_per_node, launcher="torchrun", env_vars=env_vars)
...
def create_slurm_executor(config: TrainingConfig) -> run.SlurmExecutor:
    ...
    if config.compute.tunnel_type.lower() == "ssh":
        tunnel = run.SSHTunnel(
            user=config.compute.user, host=config.compute.host, job_dir=config.compute.remote_job_dir
        )
    elif config.compute.tunnel_type.lower() == "local":
        tunnel = run.LocalTunnel(job_dir=config.compute.remote_job_dir)
    else:
