...
from nemo_automodel.checkpointing import (
    checkpoint_and_decide_exit,
    checkpoint_exists,
    load_checkpoint,
    save_checkpoint_and_time,
)
from nemo_automodel.components.state import GlobalState
from nemo_automodel.config import ConfigContainer
from nemo_automodel.utils.distributed_utils import initialize_automodel
from nemo_automodel.utils.train_utils import (
    eval_log,
    reduce_loss,
    training_log,
)
from nemo_lm.config.common import ProfilingConfig
from nemo_automodel.utils.common_utils import (
...
from nemo_automodel.utils.log_utils import setup_logging
...
def setup(
    ...
    with timers("model-and-optimizer-setup", log_level=0, barrier=True):
    ...
    if (cfg.checkpoint_config.load is not None or cfg.checkpoint_config.pretrained_checkpoint is not None) and (
        ...
        with timers("load-checkpoint", log_level=0, barrier=True):
            load_checkpoint(
                state,
                model,
                optimizer,
    ...
    # Data stuff.
    with timers("train/valid/test-data-iterators-setup", log_level=0, barrier=True):
...
def train_step(
    ...
    # Forward pass.
    with timers("forward-backward", log_level=1, barrier=model_config.barrier_with_L1_time):
    ...
    # Update parameters.
    with timers("optimizer", log_level=1, barrier=optim_config.barrier_with_L1_time):
        optimizer.step()
...
def evaluate_and_print_results(
    ...
    with timers("evaluate", log_level=0, barrier=True):
...
def train(
    ...
    while global_state.train_state.step < train_config.train_iters:
        ...
        if (
            ...
            with timers("eval-time", log_level=0, barrier=True):
