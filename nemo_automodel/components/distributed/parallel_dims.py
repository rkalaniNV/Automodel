    ...
    def _build_mesh(
        ...
        if dp_shard_cp_mesh_dim_names != []:
            mesh[tuple(dp_shard_cp_mesh_dim_names)]._flatten(
                mesh_dim_name=DimNames.DP_SHARD_CP
            )
    ...
    def build_mesh_info(self):
        ...
        for i in range(self.world_size):
            self.full_rank_info[i][DimNames.DP_CP_TP] = (
                self.full_rank_info[i][DimNames.DP_REPLICATE]
                * self.dp_shard
                * self.cp
                * self.tp
            ...
            self.full_rank_info[i][DimNames.DP_SHARD_CP] = (
                self.full_rank_info[i][DimNames.DP_SHARD] * self.cp
                + self.full_rank_info[i][DimNames.CP]
            )
